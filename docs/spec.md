...
| `/lineage` | 楽団系譜図 | レベル切り替え機能付きSVG系譜図（現在アプリ内リンクは非表示・URL直接入力でアクセス可） |
...
アルゼンチンタンゴ楽団の系譜・師弟・親交関係を3段階のレベルで表示するSVG系譜図。レベルをボタンで切り替えながら段階的に理解を深められる設計。

**アクセス方法**: URL直接入力（`/lineage`）のみ。タイトル画面・演奏家紹介ページ等からのリンクは現在非表示。

### ページ共通レイアウト

- 背景色: `#0c0101`
- フォント: Noto Serif JP（Google Fontsから動的ロード、`useEffect`内で`<link>`タグを追加）
- ヘッダー上部: `← 演奏家紹介に戻る` リンク（`/artists`）
- ヘッダー: タイトル「アルゼンチンタンゴ楽団　系譜・師弟・親交図」+ 副題「ARGENTINE TANGO ORCHESTRAS GENEALOGY　（1900 — 1998）」
- レベルボタン: 3つ並列表示（選択中: `backgroundColor:'#8B0000'`、未選択: `backgroundColor:'transparent'`, `border: 1.5px solid ${GOLD}`）
- 初期表示レベル: 1

### 表示レベル切り替え

`LEVELS` 配列で定義した3段階をボタンで切り替え。`useState(1)` で管理。

| レベルID | ボタンラベル | 内容 |
|--------|------------|------|
| 1 | レベル1：主要系統 | 6名のカードグリッド |
| 2 | レベル2：代表楽団 | SVG系統図（18ノード、15エッジ） |
| 3 | レベル3：完全版 | SVG系統図（37ノード、35エッジ） |

### レベル1（Level1コンポーネント）

`L1_CARDS` 配列（6名）をCSSグリッドで表示：
...
各カード: `border: 1.5px solid #C9A84C`、`backgroundColor: #1a0404`、`borderRadius: 10`、`padding: 18px 12px`、フォントサイズ14px

収録名: ロベルト・フィルポ / フランシスコ・カナロ / アニバル・トロイロ / カルロス・ディ・サルリ / オスバルド・プグリエーセ / アストル・ピアソラ

### レベル2・3（TreeSVGコンポーネント）

共通SVGコンポーネント `TreeSVG({ nodes, edges })` でレベル2・3を描画。
...
**SVGサイズ:** ノードのmaxCol・maxRowから動的計算（`W = (maxCol+1)*COL_W + PAD*2`、`H = (maxRow+1)*ROW_H + PAD*2 + 40`）

**ノード描画:** `<rect>` （rx=8、fill=`#1a0404`、stroke=`#C9A84C`、strokeWidth=1.2）+ 中央テキスト（ラベル・年代）

**エッジ描画:** 三次ベジェ曲線（`M x1,y1 C mx,y1 mx,y2 x2,y2`）
- 実線（solid）: stroke=`#C9A84C`、strokeWidth=1.5、opacity=0.7
- 破線（dashed）: stroke=`#8a7040`、strokeDasharray=`6,4`、strokeWidth=1.2、opacity=0.45

**凡例:** SVG下部に固定表示「━━ 直接継承　--- 影響」

**スクロール:** `overflowX: 'auto'` でコンテナ横スクロール対応

### 旧実装コードについて

`LevelFull` コンポーネント（旧インタラクティブ系譜図）がファイル内に残存しているが、どのレベルにも割り当てられておらず描画されない（デッドコード）。削除予定。
...
- **iframe内部はCSSで操作不可**: iframe内のHTML要素に直接スタイルを当てることができないため、ぼかし処理は外側の`div`に`filter: blur(6px)`を適用することで実現している
- **自動再生（autoplay）不可**: ブラウザポリシーにより、ユーザー操作なしでの音声自動再生は動作しない。クイズ画面でユーザーが再生ボタンを自分でタップする必要がある
- **30秒制限**: Spotifyへの未ログイン時は30秒で再生が自動停止し、停止後は再生ボタンが押せなくなる。この制約を前提として、操作ガイドでユーザーに明示している
