...
const ML = 140, MT = 70, MR = 60, MB = 80;
...
export default function TangoChart() {
  ...
  const onBgClick = () => {
    if (!hasMoved.current) setSelected(null);
  };
  const onNodeClick = (id) => {
    if (!hasMoved.current) setSelected(p => p === id ? null : id);
  };
  ...
  return (
    <div style={{ width: '100vw', height: '100vh', background: '#0c0101', display: 'flex', flexDirection: 'column', fontFamily: "'Noto Serif JP', serif", userSelect: 'none' }}>
      ...
      {/* ── HEADER ── */}
      <div style={{ padding: '10px 28px', borderBottom: '1px solid rgba(201,168,76,0.18)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <div>
          <div style={{ fontSize: 17, color: GOLD, letterSpacing: '0.18em', fontWeight: 700, textWrap: 'nowrap' }}>
        ...
        <div style={{ display: 'flex', gap: 20, alignItems: 'center', flexShrink: 0 }}>
          <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'rgba(245,230,200,0.55)' }}>
            <svg width={34} height={10} style={{ display: 'block' }}>
              <line x1="0" y1="5" x2="34" y2="5" stroke={GOLD} strokeWidth="1.5" />
            </svg>
            師弟関係
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: 'rgba(245,230,200,0.55)' }}>
            <svg width={34} height={10} style={{ display: 'block' }}>
              <line x1="0" y1="5" x2="34" y2="5" stroke="#8a7040" strokeWidth="1.5" strokeDasharray="6,5" />
            </svg>
            親交関係
          </span>
          <span style={{ fontSize: 10, color: 'rgba(201,168,76,0.32)', marginLeft: 10, lineHeight: 1.6, textAlign: 'right' }}>
            ドラッグ：移動　ホイール：ズーム<br/>
            楽団名クリック：関係表示
          </span>
        </div>
